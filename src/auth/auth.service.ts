...
export class AuthService {
  ...
  async verifyOtp(requestId: string, otp: string) {
    const r = await this.prisma.otpRequest.findUnique({ where: { id: requestId } });
    if (!r) throw new UnauthorizedException("OTP_INVALID");
    if (r.usedAt) throw new UnauthorizedException("OTP_USED");
    if (r.expiresAt < new Date()) throw new UnauthorizedException("OTP_EXPIRED");
    if (r.attempts >= 5) throw new UnauthorizedException("OTP_TOO_MANY_ATTEMPTS");
    ...
    const user =
      (await this.prisma.user.findUnique({ where: { phoneE164: r.phoneE164 } })) ??

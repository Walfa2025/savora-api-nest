...
export class SelfUnblockService {
  ...
  async initForUser(userId: string) {
    ...
    if (lastConfirmed?.createdAt) {
      const cooldownEndsAt = new Date(
        lastConfirmed.createdAt.getTime() + cooldownDays * 24 * 3600 * 1000,
      );
    ...
    const reference =
      'UB-' + Math.random().toString(36).toUpperCase().slice(2, 8);
  ...
  async markProof(paymentId: string, userId: string) {
    const existing = await this.prisma.penaltyPayment.findUnique({
      where: { id: paymentId },
    });
    if (!existing) throw new NotFoundException('PenaltyPayment not found');
    if (existing.userId !== userId)
      throw new NotFoundException('PenaltyPayment not found');
  ...
  async confirm(paymentId: string, bankTxnRef: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const existing = await tx.penaltyPayment.findUnique({
        where: { id: paymentId },
      });
      if (!existing) throw new NotFoundException('PenaltyPayment not found');
      ...
      if (existing.status === PenaltyStatus.CONFIRMED) {
        ...
        return {
          ...
          lastSelfUnblockAt: u?.lastSelfUnblockAt
            ? u.lastSelfUnblockAt.toISOString()
            : null,
        };
      }

      if (
        existing.status === PenaltyStatus.REJECTED ||
        existing.status === PenaltyStatus.EXPIRED
      ) {
      ...
      if (upd.count !== 1) {
        const again = await tx.penaltyPayment.findUnique({
          where: { id: paymentId },
        });
        ...
        if (again.status === PenaltyStatus.INITIATED)
          throw new ConflictException('CONFIRM_REQUIRES_PROOF');
        if (again.status === PenaltyStatus.EXPIRED)
          throw new ConflictException('CONFIRM_NOT_ALLOWED_EXPIRED');
        throw new ConflictException('CONFIRM_NOT_ALLOWED_STATE');
      }
      ...
      return {
        ...
        bankTxnRef,
        lastSelfUnblockAt: now.toISOString(),
      };
    });
  }
}
